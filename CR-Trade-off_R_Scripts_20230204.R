...
##### C. Plot Figure 1: Plot Competitiveness-Invulnerability trade-off  
...
########### Part 4: Plot Figure 2:Top-down control effect on diversity ################################################  
  ...
  mtext("A", outer=F,side=3, line=-1.3, at=1.1, cex=0.8)
  mtext("RAD decay coefficient", outer=F,side=2, line=2, cex=0.7)
  ...
  mtext("B", outer=F,side=3, line=-1.3, at=1.1, cex=0.8)
  #######################
  ######Evenness#####
  x=2 #Evenness
  l=RDA_Ev_D02
  ...
  mtext("C", outer=F,side=3, line=-1.3, at=1.1, cex=0.8)
  ...
  Xaxis=FALSE
  DiversityPlot(l,x,Ylim,LMMTable,Xaxis)
  mtext("D", outer=F,side=3, line=-1.3, at=1.1, cex=0.8)
  ...
  Xaxis=TRUE
  DiversityPlot(l,x,Ylim,LMMTable,Xaxis)
  mtext("E", outer=F,side=3, line=-1.3, at=1.1, cex=0.8)
  ...
  Xaxis=TRUE
  DiversityPlot(l,x,Ylim,LMMTable,Xaxis)
  mtext("F", outer=F,side=3, line=-1.3, at=1.1, cex=0.8)
  legend("bottomright",
...
##########################################################################################
  ...
  library(raster)
  library(rgeos)
  S1En=read.xls("Data_Environment.xlsx",sheet=2,header=T)
  newmap <- getMap(resolution = "low")
  plot(newmap, xlim = c(120, 124), col="gray90",ylim = c(21, 27), 
       asp = 1, xaxt='n',xlab="Longtitude",ylab="Latitude", cex=0.7)
  v=seq(116,126,2)
  h=seq(18,30,3)
  abline( h=h, v=v,col = "grey20",lty=3)
  labelsX=parse(text=paste(v, "^o ", "*E", sep=""))
  labelsY=parse(text=paste(h, "^o ", "*N", sep=""))
  axis(1,at=v,labels=labelsX,lwd=2)
  axis(2,at=h,labels=labelsY,lwd=2)
  axis(3,at=v,labels=FALSE,lwd=2,tck = 0)
  axis(4,at=h,labels=FALSE,lwd=2,tck = 0)
  
  points(En$Longitude, En$Latitude, pch = 19,cex =1.5)
  #L=c("st. 1 (on Apr. and Oct. 2014, Jul. 2015,and May 2016)","st. 9 (on Apr. 2014, Jul. 2015 and Oct. 2015)")
  L=c("Station 1" ,"Station 9")
  text(En$Longitude, En$Latitude,label=L, cex=0.8,pos=4)
  label=En$Station
...
############## Figure S2: Total abundance at T0 and T12 plot #####
...
############## Figure S4: Top-down control effect on community composition maintenance #####       
    ...
    StationCruiseNames=c("2014AprSt1","2014OctSt1","2014OctSt9",
                         "2015JulSt1","2015JulSt9","2016MaySt1")
    dist = avgdist(t(RawTable),3188,  dmethod = "euclidean")
    d=as.matrix(dist)
    T0=grep("T0",colnames(d), value = F)
    T0_Dist=d[,T0]
    ...
    for (i in 1:6){
      SampleID[1,i]
      T0=grep(SampleID[1,i],colnames(T0_Dist), value = F)
      T12=grep(SampleID[1,i],rownames(T0_Dist), value = F)
      ALL=T0_Dist[T12,T0]
      T12_D02=grep("DS",names(ALL), value = F)
      T12_D30=grep("D30S",names(ALL), value = F)
      Dilution_D02=sapply(strsplit(names(ALL)[T12_D02],"D"),"[",3)
      Dilution_D30=sapply(strsplit(names(ALL)[T12_D30],"D"),"[",3)
      EachSample_D02[[i]]=data.frame(ALL[T12_D02],Dilution_D02)
      EachSample_D30[[i]]=data.frame(ALL[T12_D30],Dilution_D30)
    }
    names(EachSample_D02)=SampleID[1,]
    names(EachSample_D30)=SampleID[1,]
    ...
    par(mar = c(4, 2, 0, 0)) 
    for (i in 1:6){
      D02=EachSample_D02[[i]][-nrow(EachSample_D02[[i]]),]
      D30=EachSample_D30[[i]]
      ymax=max(c(D02$ALL.T12_D02.,D30$ALL.T12_D30.))
      ymin=min(c(D02$ALL.T12_D02.,D30$ALL.T12_D30.))
      plot(D02$Dilution_D02,D02$ALL.T12_D02.,
           col="blue", ylim=c(ymin,ymax),cex.axis=0.7,xaxt='n', xlab="")
      axis(side = 1, cex.axis=0.9, 
           at = c(1,2,3,4), labels = c("25%","50%","75%","100%"), 
           tck = -0.05, line= 0)
      l_D02=lm(D02$ALL.T12_D02.~as.numeric(D02$Dilution_D02))
      if (summary(l_D02)$coefficient[2,4]<=0.05){
        L=1
      }else{L=2}
      abline(l_D02,col="blue",lty=L)
      points(D30$Dilution_D30,D30$ALL.T12_D30.)
      l_D30=lm(D30$ALL.T12_D30.~as.numeric(D30$Dilution_D30))
      if (summary(l_D30)$coefficient[2,4]<=0.05){
        L=1
      }else{L=2}
      abline(l_D30,col="black",lty=L)
      
      mD02=aov(D02$ALL.T12_D02.~as.numeric(D02$Dilution_D02))
      summary(mD02)
      mD30=aov(D30$ALL.T12_D30.~as.numeric(D30$Dilution_D30))
      summary(mD30)
      ANCOVA=anova(mD02,mD30)
      p=ANCOVA$`Pr(>F)`[1]
      
      rp = vector('expression',1)
      rp[1] = substitute(expression(italic(ANCOVA-p) == MYOTHERVALUE), 
                         list(MYOTHERVALUE = format(p, digits = 2)))[2]
      
      legend('topright', legend = rp, bty = 'n',cex=0.7)
      mtext(StationCruiseNames[i], line=0, cex=0.7, at =1.5)
      
    }
    mtext("Top-down control dilution factors",side=1, outer=T,cex=0.8, line=-0.5)
    mtext("Euclidean distance between T0 and T12",side=2, outer=T,cex=0.8, line=1)
...
############## Figure S8: Total bacterial community net growth rate plot#####   
    ...
    legend("bottomright",legend=c("protists-diluted","protists+viruses-diluted"),pch=16,col=c("black","red"), cex=0.8)
